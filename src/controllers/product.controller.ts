...
const getProducts = async (req: Request, res: Response) => {
    ...
    if (category_id) {
        ...
        const [rows, fields]: [any, any] = await promisePool.execute(query, [category_id])
...
const getProduct = async (req: Request, res: Response) => {
    ...
    const [rows, fields]: [any, any] = await promisePool.execute(query, [product_id])
...
const createProduct = async (req: Request, res: Response) => {
    ...
    if (rows.affectedRows <= 0) throw new BadRequestError('Product name already exists')
...
const deleteProduct = async (req: Request, res: Response) => {
    ...
    const [rows, fields]: [ResultSetHeader, any] = await promisePool.execute(query, [product_id])

    if (rows.affectedRows <= 0) throw new NotFoundError('Product id was not found')
